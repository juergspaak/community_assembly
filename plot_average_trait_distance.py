...
"""
species_id = af.generate_species(n_coms = 100, years = 1000)
...
for i in range(2):
    ...
    ax[0,i].plot(time, np.nanpercentile(np.nanmean(traits, axis = -1), [25,50,75], axis = 0).T,
                 color = "r")
    ax[0,i].plot(time, np.nanpercentile(np.nanmean(ref, axis = -1), [25,50,75], axis = 0).T,
                 color = "b", alpha = 0.5)
    
    # plot trait variance over time
    ax[1,i].plot(time, np.nanpercentile(np.nanstd(traits, axis = -1), [25,50,75], axis = 0).T,
                 color = "r", label = "Simulation")
    ax[1,i].plot(time, np.nanpercentile(np.nanstd(ref, axis = -1), [25,50,75], axis = 0).T,
                 color = "b", alpha = 0.5, label = "Random")
    
ax[0,0].set_title("Basal species")
ax[0,1].set_title("Predator species")

ax[0,0].set_ylabel("Mean tratis")
ax[1,0].set_ylabel("Std(traits)")
ax[1,0].set_xlabel("Time")
ax[1,1].set_xlabel("Time")

ax[1,0].legend(["Sim 75 perc.", "Sim 50 perc", "Sim 25 perc", "Rand 75", "Rand 50", "Rand 25"])
