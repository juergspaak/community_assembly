...
if False:
    ...
    n_coms = 100
    # prey only 
    species_id = ap.generate_species(n_coms, years, level = [1,0], omega = 3)
    present, equi_all, surv = ap.community_assembly(species_id, pr = False)
    np.savez(path + "data_LV_assembly.npz", equi_all = equi_all, presetn = present,
             **species_id)
    
    species_id = ap.generate_species(n_coms, years, level = [1,1], omega = 3)
    present, equi_all, surv = ap.community_assembly(species_id, pr = False)
    np.savez(path + "data_LV_assembly_predators.npz", equi_all = equi_all, presetn = present,
             **species_id)
...
ax[3,1].set_ylabel("Frequency", fontsize = fs)
...
ax[3,1].set_xlabel("Persistence time", fontsize = fs)
ax[3,2].set_xlabel("Persistence time", fontsize = fs)

for i, a in enumerate(ax[:3].flatten()):
    a.set_title("ABCDEFGHIJKLMNOP"[i], loc = "left", fontsize = fs)

ax[3,1].set_title("J", loc = "left", fontsize = fs)
ax[3,2].set_title("K", loc = "left", fontsize = fs)
